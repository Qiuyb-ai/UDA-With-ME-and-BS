...
class DACS(UDADecorator):
    ...
    def forward_train(self,
        ...
        if not self.source_only:
            ...
            img_clone,target_img_clone,gt_semantic_seg_clone,pseudo_label_clone,pseudo_weight_clone\
                =img.clone(),target_img.clone(),gt_semantic_seg.clone(),pseudo_label.clone(),pseudo_weight.clone()
            mixed_img, mixed_lbl = [None] * batch_size, [None] * batch_size
            mixed_seg_weight = pseudo_weight_clone.clone()
            mix_masks, class_choice = get_class_masks(gt_semantic_seg_clone)
            for i in range(batch_size):
                strong_parameters['mix'] = mix_masks[i]
                mixed_img[i], mixed_lbl[i] = strong_transform(
                    strong_parameters,
                    data=torch.stack((img_clone[i], target_img_clone[i])),
                    target=torch.stack(
                        (gt_semantic_seg_clone[i][0], pseudo_label_clone[i])))
                _, mixed_seg_weight[i] = strong_transform(
                    strong_parameters,
                    target=torch.stack((gt_pixel_weight[i], pseudo_weight_clone[i])))
            del gt_pixel_weight
        ...
        if self.enable_masking and self.mask_mode.startswith('separate'):
            if self.masked_mix_train:
                masked_loss = self.mic(self.get_model(), img, img_metas,
                                       gt_semantic_seg, mixed_img,
                                       target_img_metas, valid_pseudo_mask,
                                       torch.squeeze(mixed_lbl, dim=1), mixed_seg_weight, augm=False)
                seg_debug.update(self.mic.debug_output)
                masked_loss = add_prefix(masked_loss, 'mix_masked')
            ...
            if self.original_mix:
                ...
                seg_debug.update(self.mic.debug_output)
                masked_loss = add_prefix(masked_loss, 'ori_masked')
                masked_loss, masked_log_vars = self._parse_losses(masked_loss)
                log_vars.update(masked_log_vars)
                masked_loss.backward()
